...
class EthicalReasoningServer {
  ...
  private frameworkGuidance(framework: FrameworkType, _scenario: string, action: string): string {
    switch (framework) {
      case "utilitarianism":
...
const server = new Server({ name: "ethical-reasoning-server", version: "0.1.5" }, { capabilities: { tools: {} } });
