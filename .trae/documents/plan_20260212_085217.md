...
I will migrate the `collaborative-reasoning` MCP server to the new Code Mode architecture, separating concerns into
Core, Code Mode, and MCP layers.
...
- **`src/core/logic.ts`**: Extract business logic from the current server class into a `CollaborativeReasoningManager`
  class.
...
- **`src/codemode/index.ts`**: Create the `CollaborativeReasoning` class.
  ...
  - Expose a public, strongly-typed API method:
    `collaborate(data: CollaborativeReasoningData): Promise<CollaborativeReasoningData>`.
